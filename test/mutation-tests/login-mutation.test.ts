...
export function loginMutationTest(connection: AxiosInstance, mochaUser: UserInput) {
  describe('Login Mutation Test', () => {
    ...
    it('Should return an error when trying to login with an incorrect password', async () => {
      ...
      const apolloErrors = (
        await mutationLogin(connection, { email: mochaUser.email, password: 'mochauserPassword123' })
      ).errors as ApolloErrorFormat[];
